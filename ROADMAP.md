...
| FEATURE-328 | 0.7.0 | P1 | 🚧 开发中（文件写入/修改流式渲染用户体验优化：write_to_file 内容按行号增量显示（git diff 风格 `N+ 内容`），replace_in_file search/replace 逐行实时显示（search 行 `-`、replace 行 `+`），replace 块指定 start_line 时显示真实行号、未指定则不显示；OpenAI 模式 JSON arguments 值片段增加转义解码（`\n`→换行等）使换行渲染与 XML 模式一致；ToolCallRenderer 重构为行式状态机，JSON 解析器仅影响渲染值不影响原始累积；新增 toolcall_stream_test.go 单测 11 个（含 FEATURE-328 UC-0001~0006）[BUILD-366]） |

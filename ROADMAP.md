...
- [ ] **FEATURE-394 Web UI 系统设置面板对齐优化**
  ...
  - 方案（已确认）：仅修改 CSS。`.set-row` 已使用 `justify-content: space-between`，`.set-label` 占满剩余空间（flex:1）。给 `.set-label` 增加 `text-align: right`，使名称文本右对齐紧贴值控件，值控件保持左对齐，两者靠向中线。
  - 需求：① `.set-label` 增加 `text-align: right`
  - 实施：`web/static/style.css` 的 `.set-label` 增加 `text-align: right` [BUILD-492]

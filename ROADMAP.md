...
- [ ] **FEATURE-456 专职监督 LLM 交付复核**
  ...
  - 测试：见 use-case/FEATURE-456/；单元测试：`agent/supervisor_test.go`（14 个用例：parseSupervisorReview 放行/打回/JSON 容错、supervisorToolAllowed 白名单默认/自定义、hasCompletedStep 完成检测、formatReviewAsText/formatReviewFeedback 格式化、supervisorMaxRetries 默认 20/可配、supervisorEntryEnabled 三开关默认值、runSupervisorReview 禁用/入口关闭/超限强制放行/调用失败降级）[BUILD-727]

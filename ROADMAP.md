...
| FIX-317 | 0.7.0 | P1 | 🚧 开发中（OpenAI 模式 replace_in_file 执行失败死循环：FIX-314 retry 静默丢弃错误使 LLM 无法看到 path/SEARCH/JSON 错误信息而重复原样调用；需区分：JSON 解析错→静默重发，path/SEARCH/其他→回灌 formatToolError 结构化反馈让 LLM 修正） |

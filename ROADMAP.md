...
- [x] **FEATURE-388 工具调用交互标准化** ✅ 已完成
  ...
  - 测试：`agent/interaction_test.go`（Interaction 模型 + TerminalInteractionManager + 迁移后行为不变 + promptToolConfirmation/promptErrorConfirmation 构造的 Interaction 含 Keys）、`web/session_test.go`（WebIO.Ask 推送结构化 interaction + 接收 interaction_answer）、`tmp/feature388_interaction_test.js`（前端 QWERTY 虚拟键盘渲染 + 高亮可用键位 + 单键即响应 + 实体键盘映射 + 补充指令用主输入框 + 左右布局 + 空格键 + 右侧标注连接线，34 断言全过）、`agent/tool_summary_test.go`（ToolSummary 结构化 + EventToolCall 携带摘要）；`go build/vet/test ./...` 全绿；浏览器验证确认放行/错误处理/ESC中断/提问选择统一到 QWERTY 虚拟键盘（A/G/D/C 字母键 + 数字键映射放行次数/选项），点击虚拟键或按实体键盘对应键立即响应，补充指令用主输入框；确认提示信息与虚拟键盘左右布局（提示在左、键盘在右），提升横向空间利用率、降低纵向占用；底部行 Space（预留扩展）+ Enter；按键说明在虚拟键盘右侧纵向排列，每个说明用连接线指向对应键，直观无需上下对照 [BUILD-475]

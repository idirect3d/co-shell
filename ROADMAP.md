...
- [ ] FEATURE-208 外部工具（bin/）梳理与优化：
  - [ ] 新增 pdf2png 工具：将 PDF 拆分为分页 PNG 图片，支持 LLM 多模态 PDF 内容解析
  - [ ] 新增 docx2pdf 工具：将 .docx 文件转换为 PDF，支持打印、转 PNG 视觉识别
  - [ ] 新增 doc2pdf 工具：将老式 .doc 文件转换为 PDF，支持打印、转 PNG 视觉识别
  - [ ] 为每个外部工具创建同名参数说明文件（.md），明确用途、参数、使用示例
  - [ ] 系统提示词中新增"外部工具"专用说明节（ExternalTools），指导 LLM 调用 bin/ 下的工具

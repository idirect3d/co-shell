...
- [x] **FEATURE-446 Web UI 区块边界导航箭头位置调整** ✅ 已完成 [BUILD-680]
  - 背景：主消息区的区块边界导航小箭头（blockNavTop/blockNavBottom，FEATURE-419）当前位于顶部/底部居中位置，用户上下切换定位区块边界时需要移动较大距离。用户希望把它们移到靠近右边滚动条、且靠近垂直方向中间的位置，使上下切换更便捷。
  - 方案（已确认）：将 `.block-nav.top` 从 `top:10px; left:50%` 改为 `right:8px; top:50%; transform:translateY(-115%)`（中间偏上），`.block-nav.bottom` 从 `bottom:10px; left:50%` 改为 `right:8px; bottom:50%; transform:translateY(115%)`（中间偏下），两者都靠近右边滚动条。
  - 实施：① `web/static/style.css` 修改 `.block-nav.top`/`.block-nav.bottom` 定位（right:8px + 垂直中间偏上/偏下）；② 测试用例

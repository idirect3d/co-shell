...
## I 组：LLM 弹窗 / 窗口模式（后端持续运行时驱动前端）

> 能力定义：LLM 可弹出一个**常驻浮层窗口**（非聊天流内的块），把组件树渲染进窗口；窗口**不阻塞**回合，LLM 在“后台”继续跑并持续把中间结果推回窗口；用户在窗口里的操作能回到 LLM。
> 本期范围（用户已拍板）：单窗口、位置固定、仅关闭按钮（不拖动/不最小化/不持久化）；`ui_window` 只管开关，内容由 `render_ui`/`ui_update` 写入；action 默认非阻塞、可声明 `blocking:true` 走挂起等待。

### UC-50 打开窗口并写入内容

- **步骤**：让 agent 调用 `ui_window(action="open", title="用户信息采集")`，随后 `render_ui(target="window", tree=...)` 渲染含表单的组件树。
- **预期**：① 页面出现常驻浮层窗口（固定位置、标题栏含标题与关闭按钮）；② 窗口内渲染出组件树；③ 聊天流**不出现**该树；④ 控制台无错误。

### UC-51 窗口内容原地更新（回合运行中）

- **步骤**：承接 UC-50，在**同一回合内**连续 `render_ui(target="window", update="<ui-id>")`（如进度条 30%→60%→100%，或表格逐行追加）。
- **预期**：① 窗口内容原地替换，窗口不重建、不闪烁；② 被更新节点 `data-ui-id`、父节点、索引保持不变；③ 更新期间窗口保持打开，且不阻塞 LLM 的后续工具调用。

### UC-52 非阻塞：用户操作即时注入当前回合

- **步骤**：在 UC-50 的窗口表单里填入数据并提交（action 未声明 blocking）。
- **预期**：① 当前回合**不被打断、不结束**；② 该操作以结构化载荷在**下一次 LLM 调用前**注入本回合；③ LLM 后续输出/渲染体现该数据；④ 窗口保持打开（不自动关闭）。

### UC-53 可选阻塞：`blocking:true` 时挂起等待

- **步骤**：窗口内表单的提交 action 声明 `blocking:true`，且 agent 以 `render_ui(waiting=true)` 模式使用（沿用现有机制）。
- **预期**：① 用户提交后作为**工具结果**返回，同一回合继续；② 窗口仍在；③ 与 UC-52 的非阻塞路径互不干扰，无死锁（专项风险点）。

### UC-54 关闭窗口与生命周期

- **步骤**：① 点击窗口关闭按钮；② 再让 agent 调 `ui_window(action="close")`；③ 切换会话；④ 刷新页面。
- **预期**：① 两种关闭方式均使窗口消失；② 刷新/切会话后窗口**不残留**（本期不持久化）；③ 窗口关闭后对窗口内 ui-id 的 `ui_update` 不抛异常（安全 no-op + `console.warn`）。

### UC-55 单窗口约束与重复打开

- **步骤**：窗口已打开时再次 `ui_window(action="open", title="新标题")`。
- **预期**：① 仍只存在一个窗口（复用同一实例、标题更新），不叠加多个浮层；② 回执/日志说明为“复用已有窗口”。

### UC-56 窗口内组件交互与终端降级

- **步骤**：① 窗口内放 `row/col + chart + form` 并交互（下拉/数字/勾选/提交）；② 在终端 TUI 出口让 agent 调 `ui_window`。
- **预期**：① 窗口内取值正确、`ui_action` 上行含正确 `ui_id`/`action_id` 与结构化 payload；② 终端不报错、不卡死，只输出降级文本，`--output-format json` 下事件仍可序列化。

### 本期明确不做（避免误解为缺陷）

- 多窗口、拖动/缩放/最小化、窗口位置/尺寸自定义；刷新与切会话后的窗口恢复；窗口自身的阻塞（阻塞仍由 `render_ui(waiting=true)` 承担）。

...
buildSystemPrompt(rules string)                          → 默认模式（ResultModeMinimal）
    └── buildSystemPromptWithMode(rules, mode, name, desc, principles, userName, channel, toolUsageKey)
```
...
Agent.New() / Agent.SetConfig() / Agent.SetResultMode() / Agent.rebuildSystemPrompt()
    └── buildSystemPromptWithMode(rules, mode, agentName, agentDescription, agentPrinciples, userName, channel, toolUsageKey)
```

其中 `toolUsageKey` 参数由 `rebuildSystemPrompt()` 根据当前 `toolCallModeMgr` 的状态决定：
- OpenAI 模式：`toolUsageKey = ""`（使用 `i18n.T(KeySystemPromptToolUsage)`）
- XML 模式：`toolUsageKey = "xml"`（使用 `BuildToolUsagePrompt()` 生成 XML 格式）
...
Part 2 的内容根据 **ToolCallMode** 动态生成，支持两种模式：

| 模式 | 行为 | 说明 |
|------|------|------|
| `openai`（默认） | 使用 `i18n.T(KeySystemPromptToolUsage)` | 标准文本描述，配合 API 的 `tools` 参数使用 |
| `xml` | 使用 `BuildToolUsagePrompt(ToolCallModeXML, tools, lang)` | 生成详细的 XML 格式工具调用说明，不发送 `tools` 参数 |

- **OpenAI 模式**: 详细描述每个可用工具的用途、使用场景和示例，覆盖所有内置工具和 MCP 工具
- **XML 模式**: 生成 XML 格式的工具定义，LLM 在 content 中嵌入 `<tool_call>` XML 标签来调用工具，无需 API 级别的 `tools` 参数
- **目的**: 帮助 LLM 理解每个工具的用途，减少试错，提高工具调用准确率
- **FEATURE-182**: XML 模式适用于不支持 function calling 的 LLM API，通过内嵌 XML 标签实现工具调用
...
func buildSystemPromptWithMode(
    ...
    toolUsageKey string,    // 工具使用指南 i18n Key（FEATURE-182: 根据 ToolCallMode 选择）
) string
```
...
3. **构建工具使用指南**: 根据 `toolUsageKey` 参数决定：
   - 如果 `toolUsageKey` 为空字符串（OpenAI 模式）：使用 `i18n.T(KeySystemPromptToolUsage)` → 标准文本描述
   - 如果 `toolUsageKey` 非空（XML 模式）：使用 `BuildToolUsagePrompt(ToolCallModeXML, tools, lang)` → 生成 XML 格式工具定义
   - **FEATURE-182**: XML 模式生成详细的 XML 标签格式说明，LLM 通过 `<tool_call>` 标签调用工具
...
| `ToolCallModeMgr` | `agent/toolcall_mode.go` | 300+ | 工具调用模式管理器 |
| `ParseXMLToolCalls()` | `agent/toolcall_mode.go` | 168 | XML 模式工具调用解析 |
| `BuildToolUsagePrompt()` | `agent/toolcall_mode.go` | 200+ | XML 模式系统提示词生成 |
| `ToolCallModeOpenAI` | `agent/toolcall_mode.go` | 8 | OpenAI 标准模式常量 |
| `ToolCallModeXML` | `agent/toolcall_mode.go` | 9 | XML 内嵌标签模式常量 |
| `rebuildSystemPrompt()` (ToolCallMode) | `agent/agent.go` | 275-299 | 根据 ToolCallMode 选择 toolUsageKey |
| `buildTools()` (ToolCallMode) | `agent/tools.go` | - | XML 模式不发送 tools 参数 |
| `nonStreamingFallback()` (XML) | `agent/loop.go` | 280-290 | XML 模式解析工具调用 |
| `streamLLMResponse()` (XML) | `agent/stream_response.go` | 180-190 | 流式 XML 模式解析 |
| `Agent.Run()` (XML) | `agent/run.go` | 110-120 | 非流式 XML 模式解析 |
| `.set tool mode` | `cmd/settings_tool.go` | - | 运行时切换工具调用模式 |
| `--toolcall-mode` | `main.go` | - | CLI 参数指定工具调用模式 |

...
// the Web UI. It exposes all :set subcommands so the settings panel is complete.
func (h *SettingsHandler) SettingsJSON() []WebSettingGroup {
	cfg := h.cfg
	llm := cfg.LLM

	// Group 1: Model & Sampling
	modelGroup := []WebSettingItem{
		...
		{Key: "max-retries", Value: strconv.Itoa(llm.MaxRetries), Desc: i18n.T(i18n.KeyCol3MaxRetries), Type: "number"},
		...
		{Key: "top-p", Value: fmt.Sprintf("%.2f", llm.TopP), Desc: "Top-p sampling", Type: "number"},
		{Key: "top-k", Value: strconv.Itoa(llm.TopK), Desc: "Top-k sampling", Type: "number"},
		{Key: "repetition-penalty", Value: fmt.Sprintf("%.2f", llm.RepetitionPenalty), Desc: "Repetition penalty", Type: "number"},
		{Key: "max-model-len", Value: strconv.Itoa(llm.MaxModelLen), Desc: i18n.T(i18n.KeyCol3MaxModelLen), Type: "number"},
	...
	agentGroup := []WebSettingItem{
		{Key: "name", Value: agentNameValue(llm.AgentName), Desc: i18n.T(i18n.KeyCol3Name), Type: "string"},
		...
		{Key: "read-file-max-size", Value: strconv.Itoa(llm.ReadFileMaxSize), Desc: "read_file max bytes", Type: "number"},
	...
	safetyGroup := []WebSettingItem{
		...
		{Key: "loop-temp-enabled", Value: boolStr(llm.LoopTempEnabled), Desc: "loop temp adjust", Type: "bool"},
		...
		{Key: "duplicate-content-threshold", Value: fmt.Sprintf("%.2f", llm.DuplicateContentThreshold), Desc: "0.0-1.0", Type: "number"},
	...
	return []WebSettingGroup{
		{Title: i18n.T(i18n.KeySettingsGroupModel), Items: modelGroup},
		{Title: i18n.T(i18n.KeySettingsGroupDisplay), Items: displayGroup},
		{Title: i18n.T(i18n.KeySettingsGroupIdentity), Items: agentGroup},
		{Title: i18n.T(i18n.KeySettingsGroupSafety), Items: safetyGroup},
		{Title: i18n.T(i18n.KeySettingsGroupMemory), Items: memoryGroup},
		{Title: i18n.T(i18n.KeySettingsGroupSearchDebug), Items: searchGroup},
	}
}
...
// agentNameValue normalizes the agent name.
func agentNameValue(v string) string {
	if v == "" {
		return "co-shell"
	}
	return v
